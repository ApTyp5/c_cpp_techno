...
void *summer(void *sumArgs) {
    ...
    bigVector_t sum = {0};
    for (size_t i = 0; i < summerArgs_ptr->size; i++)
...
inline void start_all_threads(summerArgs args[], pthread_t threads[],
                              littleVector_t vectors[], size_t core_num, size_t el_per_core, size_t size){
    ...
    pthread_create(threads + last_i, nullptr, summer, args);
}
